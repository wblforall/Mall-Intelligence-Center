...
class EventLoyaltyRealisasiModel extends Model
{
    ...
    // Cumulative totals up to (and including) $upToMonth, keyed by program_id
    public function getCumulativeByPrograms(string $upToMonth, array $programIds): array
    {
        if (empty($programIds)) return [];
        $rows = $this->db->table('event_loyalty_realisasi')
            ->select("program_id, SUM(jumlah) as total_jumlah, COALESCE(SUM(member_aktif), 0) as total_member_aktif")
            ->whereIn('program_id', $programIds)
