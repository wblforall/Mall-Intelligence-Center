...
class AdminHolidays extends BaseController
{
    ...
    // Tarik otomatis dari feed kalender libur nasional Indonesia (Google ICS)
    public function syncApi()
    {
        if (! $this->isAdmin()) return redirect()->to('/');

        $year = (int)($this->request->getPost('year') ?? date('Y'));
        $res  = (new \App\Services\HolidaySyncService())->sync($year);

        if (! $res['ok']) {
            return redirect()->to("admin/holidays?year={$year}")
                ->with('error', 'Gagal mengambil data dari kalender. Coba lagi atau gunakan Import Massal.');
        }

        ActivityLog::write('create', 'public_holiday', null,
            "sync kalender {$year}: +{$res['inserted']} libur ({$res['skipped']} sudah ada)");

        return redirect()->to("admin/holidays?year={$year}")
            ->with('success', "Sinkronisasi {$year} selesai: {$res['inserted']} hari libur baru ditambahkan, {$res['skipped']} dilewati (sudah ada).");
    }
}
