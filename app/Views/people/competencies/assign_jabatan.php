...
<div class="d-flex align-items-center gap-3 mb-4">
    ...
    <div>
        <div class="text-muted small">Pemetaan Kompetensi → per Jabatan</div>
        <h5 class="fw-bold mb-0">
            <?= esc($jabatan['nama']) ?>
            <?php if ($jabatan['grade']): ?>
            <span class="badge bg-primary-subtle text-primary ms-1" style="font-size:.65rem"><?= esc($jabatan['grade']) ?></span>
            <?php endif; ?>
        </h5>
    </div>
...
<div class="alert alert-info py-2 small mb-3">
    ...
    Kompetensi yang di-assign di sini akan digunakan pada form TNA untuk karyawan dengan jabatan
    <strong><?= esc($jabatan['nama']) ?></strong>, menggantikan pemetaan per-departemen.
</div>
...
<form method="POST" action="<?= base_url('people/competencies/jabatan/' . $jabatan['id'] . '/assign') ?>">
    <?= csrf_field() ?>
