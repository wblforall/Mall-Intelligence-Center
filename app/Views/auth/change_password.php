...
<title>Buat Password Baru — Mall Intelligence Center</title>
...
<div class="container" style="max-width:460px">
    ...
    <div class="card p-4">
        <div class="mb-3">
            ...
            <div class="text-muted small">Password default Anda perlu diganti sebelum melanjutkan. Pastikan password memenuhi standar keamanan di bawah.</div>
        </div>
        ...
        <form method="post" action="<?= base_url('change-password') ?>" id="pwForm">
            <?= csrf_field() ?>
            <div class="mb-3">
...
pw2.addEventListener('input', () => {
    const allOk = Object.values(rules).every(fn => fn(pw.value));
    checkMatch(allOk);
});
...
document.getElementById('togglePw').addEventListener('click', function() {
    ...
    pw.type = type;
    this.querySelector('i').className = 'bi bi-eye' + (type === 'text' ? '-slash' : '');
});
document.getElementById('togglePw2').addEventListener('click', function() {
    const type = pw2.type === 'password' ? 'text' : 'password';
    pw2.type = type;
    this.querySelector('i').className = 'bi bi-eye' + (type === 'text' ? '-slash' : '');
});
</script>
