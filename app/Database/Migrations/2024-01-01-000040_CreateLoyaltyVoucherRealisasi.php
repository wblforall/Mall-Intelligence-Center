<?php

namespace App\Database\Migrations;

use CodeIgniter\Database\Migration;

class CreateLoyaltyVoucherRealisasi extends Migration
{
    public function up(): void
    {
        $this->forge->addField([
            'id'         => ['type' => 'INT', 'auto_increment' => true],
            'program_id' => ['type' => 'INT', 'null' => false],
            'item_id'    => ['type' => 'INT', 'null' => false],
            'tanggal'    => ['type' => 'DATE', 'null' => false],
            'tersebar'   => ['type' => 'INT', 'default' => 0],
            'terpakai'   => ['type' => 'INT', 'default' => 0],
            'catatan'    => ['type' => 'TEXT', 'null' => true],
            'created_by' => ['type' => 'INT', 'null' => false],
            'created_at' => ['type' => 'DATETIME', 'null' => true],
            'updated_at' => ['type' => 'DATETIME', 'null' => true],
        ]);
        $this->forge->addKey('id', true);
        $this->forge->addKey(['program_id', 'item_id', 'tanggal']);
        $this->forge->createTable('loyalty_voucher_realisasi');
    }

    public function down(): void
    {
        $this->forge->dropTable('loyalty_voucher_realisasi', true);
    }
}
